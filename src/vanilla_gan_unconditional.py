...
class generator_nocond(nn.Module):
    ...
    def __init__(self, latent_dims,
        ...
        super(generator_nocond, self).__init__()
        ...
        self.input_dims = self.latent_dims 
...
def WGAN_GP_model_nocond(latent_dims,
                vector_dims,
                numerical_dims, 
                vocab_sizes, 
                generator_dims, 
    ...
    gen  = generator_nocond(latent_dims,
               numerical_dims, 
               vocab_sizes,  
               generator_dims, 
               negative_slope, is_bn)
        
    disc = discriminator_nocond(vector_dims,
                                numerical_dims, 
                                vocab_sizes,  
                                discriminator_dims, 
                                negative_slope, is_bn)
...
class WGAN_GP_nocond():
    ...
    def build_WGAN_GP_nocond(self):

        # to do: fix bug 
        self.numerical_dims = []
        self.gen, self.disc = WGAN_GP_model_nocond(self.latent_dims, self.input_dims, self.numerical_dims, 
                self.vocab_sizes, 
                self.generator_dims, 
    ...
    def gradient_penalty(self, real_data, fake_data):
        ...
        disc_inter_outputs = self.disc(disc_inter_inputs)
    ...
    def train_disc(self, x, z):
        ...
        gen_inputs = z
        gen_outputs = self.gen(gen_inputs)
        ...
        disc_fake = self.disc(gen_outputs)
        disc_true = self.disc(x)

        # compute loss
        disc_loss, disc_real_loss, disc_fake_loss = D_loss(disc_true, disc_fake)
        gp = self.gradient_penalty(x, gen_outputs)
    ...
    def generate_samples(self, x_GE):
        ...
        for epoch in range(epochs):
            ...
            if val:                        
                    if (epoch+1) == epochs:
                        ...
                        torch.save(self.gen.state_dict(), os.path.join(self.results_dire,'generator_last_epoch.pt'))
                        torch.save(self.disc.state_dict(), os.path.join(self.results_dire,'discriminator_last_epoch.pt'))
                    ...
                    #metrics = compute_evaluation_metrics(data_real, data_gen, all_real, all_gen)
                        ...
                        for run in range(n_runs):
                            ...
                            all_real, all_gen=  self.test(test_data)
...
def parse_args():
    ...
    parser.add_argument('--output_path', type=str, default='/Experiments/carlos00/iciap/gan/vanilla_gan', help='path to save the model')
