...
class generator(nn.Module):
    ...
    def __init__(self, latent_dims, embedding_dims, generator_dims, text_embedding_dims=768, 
        ...
        self.patches_transformer_layer = nn.TransformerEncoderLayer(
            ...
            dropout=0.1, activation='relu', batch_first=True, bias=False)
    ...
    def forward(self, x, text_embedding, patches, padding_mask):
        ...
        gamma_beta = self.film_generator(text_embedding)
        ...
        modulated_patches = gamma * patches + beta

        modulated_patches = self.patches_encoder(modulated_patches)

        patches_with_cls = torch.cat((self.patches_cls_token.expand(modulated_patches.size(0), -1, -1), modulated_patches), dim=1)
        padding_mask = torch.cat((padding_mask.new_zeros(patches_with_cls.size(0), 1, dtype=torch.bool), padding_mask), dim=1)
        patches_with_cls = self.patches_transformer(patches_with_cls, src_key_padding_mask=padding_mask)
        conditioning_vector = patches_with_cls[:, 0, :self.embedding_dims]  # CLS token
        
        x_encoded = torch.cat((x,conditioning_vector), dim=1)
    ...
    def generate_samples_all(self, data_loader, num_repeats=1, balanced=False, balanced_max_oversample=5):
        ...
        if balanced:
            ...
            for i, batch in enumerate(data_loader):
                ...
                all_primary_site_array.append(batch[5].numpy())
                ...
                for disease_type in unique_disease_types:
                    ...
                    for start_idx in range(0, len(disease_indices), 64):
                        ...
                        for idx in indices:
                            ...
                            primary_site.append(dataset[idx][5].item())
            ...
            for i in tqdm(range(num_repeats), desc=f"Generating samples ({num_repeats} repeats)", total=num_repeats, unit="repeat"):
                for batch in data_loader:
                    ...
                    primary_site = batch[5].to(self.device)
