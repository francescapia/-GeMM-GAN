...
class generator(nn.Module):
    ...
    def __init__(self, latent_dims, input_embedding_dims,
                embedding_dims,
               generator_dims,
               condition_type='text', # text, image or both
        ...
        assert condition_type in ['text', 'image', 'both'], "Condition type must be either 'text' or 'image' or 'both'"
        ...
        self.encoder = nn.Linear(self.input_embedding_dims, self.embedding_dims)



      
    def forward(self, x, text_embedding, patches, padding_mask):
        # z: latent noise vector [B, latent_dims]
        ...
        if self.condition_type == 'text':
            ...
            condition_vector = self.encoder(patches) * (~padding_mask).unsqueeze(-1).float()
            condition_vector = condition_vector.sum(dim=1) / (~padding_mask).sum(dim=1, keepdim=True)
        
        x_encoded = torch.cat((x, condition_vector), dim=1)
...
class discriminator(nn.Module):
    ...
    def forward(self, gene_expression, text_embedding, patches, padding_mask): 
        ...
        x_encoded = torch.cat((gene_expression, condition_vector), dim=1)
...
def WGAN_GP_model(latent_dims,
    ...
    disc = discriminator(vector_dims, input_embedding_dims,
                                embedding_dims,
                                discriminator_dims,
                                condition_type,
                                negative_slope, is_bn)
...
class WGAN_GP():
    ...
    def __init__(self, input_dims, latent_dims,
                ...
                generator_dims,
                discriminator_dims,
                input_embedding_dims=256,
                condition_on='text', # text, image or both
                negative_slope = 0.0, is_bn= False,
                lr_d = 5e-4, lr_g = 5e-4, optimizer='rms_prop',
                gp_weight = 10,
                p_aug=0, norm_scale=0.5, train=True,
                n_critic = 5,
                freq_print= 2, freq_compute_test = 100, freq_visualize_test=100, patience=10,
    ...
    def build_WGAN_GP(self):
        ...
        # to do: fix bug
        self.numerical_dims = []
        self.gen, self.disc = WGAN_GP_model(self.latent_dims, self.input_dims,
                self.input_embedding_dims,
                self.embedding_dims,
                self.generator_dims,
       ...
       # early_stopper = EarlyStopper_gan(patience=self.patience, min_delta=0)
        for epoch in range(epochs):
            ...
            for i, data in enumerate(train_data):
                ...
                text_embedding = data[0].to(self.device)       # shape: [B, 128]
                gene_expression = data[1].to(self.device)      # shape: [B, 18868]
                patches = data[2].to(self.device)              # shape: [B, 256, 128]
                padding_mask = data[3].to(self.device)         # shape: [B, 256]
            ...
            if val:       
                ...
                if (epoch+1) % self.freq_compute_test == 0:
                    ...
                    all_real, all_gen, testing_disease_type_real, testing_disease_type_gen =  self.generate_samples_all(val_data)
...
def parse_args():
    ...
    parser.add_argument('--condition_type', type=str, default='text', choices=['text', 'image'], help='Condition type for the model')
...
if __name__ == '__main__':
    ...
    model = WGAN_GP(
        ...
        condition_on=args.condition_type,
        input_embedding_dims=768 if args.condition_type == 'text' else 1024,
