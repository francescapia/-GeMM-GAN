...
import pickle
from multi_patch_gan_dataloader import seed_worker, split_data, standardize, min_max


class MultiPatchMultiTokenGANDataset(Dataset):
    def __init__(self, case_ids, tokens_path, patches_path, gene_expressions, disease_types, primary_site, num_patches=256):
    ...
    def __getitem__(self, idx):
        ...
        token_embeddings = np.load(self.tokens_path / f'{case_id}.npy')
        token_embeddings = torch.tensor(token_embeddings, dtype=torch.float32).squeeze(0)
        token_attention_mask = np.load(self.tokens_path / f'{case_id}_attention_mask.npy')
        token_attention_mask = torch.tensor(token_attention_mask, dtype=torch.bool).squeeze(0)
        # hugging face uses 0 for padding, so we invert it for torch multi head attention layer
        ...
        return token_embeddings, token_attention_mask, gene_expression, patches, padding_mask, disease_type, primary_site
...
def dataloader_multi_patch_conditional_gan(
    ...
    token_embeddings_folder: str = None
):
    ...
    if token_embeddings_folder is None:
        token_embeddings_folder = f'../text_embeddings_contrastive_{embedding_dim}'
    ...
    train_dataset = MultiPatchMultiTokenGANDataset(
        train_case_ids, dataset_path / token_embeddings_folder, dataset_path / patch_embeddings_folder,
        gene_expressions_train, encoded_disease_types_train, encoded_primary_sites_train, num_patches=num_patches)

    validation_dataset = MultiPatchMultiTokenGANDataset(
        validation_case_ids, dataset_path / token_embeddings_folder, dataset_path / patch_embeddings_folder, 
        gene_expressions_validation, encoded_disease_types_validation, encoded_primary_sites_validation, num_patches=num_patches)
    
    test_dataset = MultiPatchMultiTokenGANDataset(
        test_case_ids, dataset_path / token_embeddings_folder, dataset_path / patch_embeddings_folder,
